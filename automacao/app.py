...
for key in ["texto_pdf", "meta_arquivos", "dados_extraidos", "dados_ibge", "dados_capag",
            "dados_legis", "dados_idh", "conformidade", "capag_result",
            "sumario", "analise", "checklist", "parecer_md",
...
tab_rapido, tab1, tab2, tab3, tab4, tab_bib = st.tabs([
    "Submissão Rápida",
    "1. Upload do PDF",
...
with tab_rapido:
    ...
    _tipos_aceitos = list(_EXTENSOES_SUPORTADAS.keys())
    _tipos_label = "PDF, Word (.docx), HTML, Planilhas (.xlsx/.csv), KML/KMZ, Imagens (.jpg/.png/.tiff)"

    st.info(texto_info_submissao_rapida(_tipos_label))

    col_up, col_mun = st.columns([1, 1])

    with col_up:
        uploads_rapido = st.file_uploader(
            f"Arquivos da proposta ({_tipos_label}):",
            type=_tipos_aceitos,
    ...
    if st.button("Analisar e gerar parecer", type="primary", key="rapido_analisar"):
        if not uploads_rapido:
            ...
            # Badges Planalto (1 por norma)
            _p_cols = st.columns(len(NORMAS_PLANALTO))
            for _col, (_chave, _meta) in zip(_p_cols, NORMAS_PLANALTO.items()):
                _norma = planalto.normas.get(_chave)
                _ok = _norma and _norma.acessivel
                _cache = " (cache)" if (_norma and _norma.cache_usado) else ""
                _col.metric(_meta["sigla"], f"{'✅' if _ok else '⚠️'} {'OK' + _cache if _ok else 'Indisponível'}")
        ...
        if st.button(f"Gerar seções via {_llm_tab3}", type="primary"):
            if st.session_state.conformidade is None:
                ...
                analise = chamada3_analise(
                    dados, conf_txt, capag_info, llm_config
                )
                st.session_state.analise = analise
...
# ---------------------------------------------------------------------------
# Aba: Biblioteca & Feedback
# ---------------------------------------------------------------------------

with tab_bib:
    st.header("📚 Biblioteca de Pareceres & Feedback")
    st.caption(
        "Repositório de pareceres validados que orientam a IA, "
        "e canal para sugestões de melhoria da equipe."
    )

    col_bib, col_fb = st.columns([1, 1], gap="large")

    # ---- Coluna esquerda: Biblioteca ----
    with col_bib:
        st.subheader("Pareceres Validados")
        st.markdown(
            "Pareceres aprovados aqui são usados automaticamente como **referência de estilo** "
            "para a IA em todas as novas análises. Quanto mais exemplos, melhor o resultado."
        )

        info = info_biblioteca()
        m1, m2 = st.columns(2)
        m1.metric("Pareceres na biblioteca", info["total_pareceres"])
        m2.metric(
            "Volume de referência",
            f"{round(info['total_chars'] / 1000)} mil chars" if info["total_chars"] else "—",
        )

        # Upload de novo parecer
        st.divider()
        st.markdown("#### Adicionar parecer validado")
        st.caption(
            f"Aceita os mesmos formatos da Submissão Rápida: "
            f"**{_tipos_label}** e também **.md**. "
            "O texto extraído é salvo como referência de estilo para a IA."
        )
        arquivos_bib = st.file_uploader(
            "Selecione até 10 arquivos de pareceres:",
            type=_tipos_aceitos + ["md"],
            accept_multiple_files=True,
            key="upload_biblioteca",
        )
        if arquivos_bib and len(arquivos_bib) > 10:
            st.warning("Máximo de 10 arquivos por vez. Apenas os 10 primeiros serão processados.")
            arquivos_bib = arquivos_bib[:10]

        if st.button("Salvar na biblioteca", key="btn_salvar_bib"):
            if not arquivos_bib:
                st.warning("Selecione pelo menos um arquivo.")
            else:
                salvos, erros = [], []
                barra = st.progress(0, text="Processando arquivos...")
                for i, arq in enumerate(arquivos_bib):
                    barra.progress((i + 1) / len(arquivos_bib),
                                   text=f"Extraindo {arq.name} ({i+1}/{len(arquivos_bib)})...")
                    ext = arq.name.rsplit(".", 1)[-1].lower()
                    # Nome automático: remove extensão e substitui espaços/pontos
                    ...
                    try:
                        ...
                        if conteudo and conteudo.strip():
                            destino = salvar_parecer_na_biblioteca(nome_auto, conteudo)
                            salvos.append(f"**{destino.name}** ({len(conteudo):,} chars)".replace(",", "."))
                        else:
                            erros.append(f"{arq.name} — texto vazio ou não extraível")
                    except Exception as e:
                        erros.append(f"{arq.name} — {e}")
                barra.empty()
                if salvos:
                    st.success(f"{len(salvos)} parecer(es) salvo(s) na biblioteca:\n" + "\n".join(f"- {s}" for s in salvos))
                if erros:
                    st.error("Falha em:\n" + "\n".join(f"- {e}" for e in erros))
                if salvos:
                    st.rerun()

        # Salvar o parecer gerado na sessão atual direto na biblioteca
        if st.session_state.get("parecer_md"):
            st.divider()
            st.markdown("#### Salvar o parecer desta sessão")
            mun_atual = (st.session_state.get("dados_extraidos") or {}).get("municipio", "municipio")
            nome_auto = st.text_input(
                "Nome para salvar:",
                value=f"Parecer_{mun_atual.replace(' ', '_')}_{datetime.now().strftime('%Y%m')}",
                key="nome_auto_bib",
            )
            if st.button("Salvar parecer atual na biblioteca", key="btn_salvar_atual"):
                if nome_auto.strip():
                    salvar_parecer_na_biblioteca(nome_auto.strip(), st.session_state.parecer_md)
                    st.success("Parecer adicionado à biblioteca!")
                    st.rerun()

        # Lista de pareceres existentes
        st.divider()
        st.markdown("#### Pareceres na biblioteca")
        arquivos = info["arquivos"]
        if not arquivos:
            st.info(
                "Biblioteca vazia. Adicione pareceres aprovados acima para melhorar a qualidade da IA."
            )
        else:
            for arq in arquivos:
                col_nome, col_info, col_del = st.columns([3, 2, 1])
                col_nome.markdown(f"📄 **{arq['nome']}**")
                col_info.caption(f"{arq['tamanho_kb']} KB · {arq['modificado']}")
                if col_del.button("🗑️", key=f"del_{arq['nome']}", help="Remover da biblioteca"):
                    remover_parecer(arq["nome"])
                    st.rerun()

    # ---- Coluna direita: Feedback ----
    with col_fb:
        st.subheader("Feedback & Sugestões")
        st.markdown(
            "Registre observações sobre a qualidade dos pareceres gerados. "
            "Os feedbacks são salvos localmente e ajudam a orientar melhorias no sistema."
        )
        ...
        st.divider()
        st.markdown("#### Registrar novo feedback")

        municipio_fb = st.text_input(
            "Município / proposta analisada:",
        ...
        comentario_fb = st.text_area(
            ...
            placeholder="Ex: A seção de análise poderia citar com mais precisão os requisitos da Lei nº 12.431/2012 e da regulamentação aplicável às debêntures incentivadas...",
            height=130,
            key="fb_comentario",
        )
        autor_fb = st.text_input(
            "Seu nome (opcional):",
            placeholder="Analista responsável",
            key="fb_autor",
        )

        if st.button("Enviar feedback", type="primary", key="btn_feedback"):
            if comentario_fb.strip():
                provedor = llm_config.provider if "llm_config" in dir() else ""
                ok = salvar_feedback(
                    municipio=municipio_fb,
                    secao=secao_fb,
                    avaliacao=avaliacao_fb,
                    comentario=comentario_fb,
                    autor=autor_fb,
                    provedor_ia=provedor,
                )
                if ok:
                    st.success("Feedback registrado! Obrigado pela contribuição.")
                    st.rerun()
                else:
                    st.error("Não foi possível salvar o feedback.")
            else:
                st.warning("Por favor, escreva um comentário antes de enviar.")

        # Histórico de feedbacks
        feedbacks = listar_feedbacks(limite=20)
        if feedbacks:
            st.divider()
            ...
            for fb in feedbacks:
                with st.expander(
                    ...
                    f"{fb.get('municipio', '—')} · {fb.get('secao', '—')} · "
                    f"{fb.get('timestamp', '')[:10]}",
                    expanded=False,
                ):
                    ...
                    meta = []
                    if fb.get("autor"):
                        meta.append(f"Analista: {fb['autor']}")
                    if fb.get("provedor_ia"):
                        meta.append(f"IA: {fb['provedor_ia']}")
                    if meta:
