"""Gerador de Portarias — fluxo espelhado do Gerador de Pareceres."""
...
from llm_client import LLMConfig, DESCRICAO_PRIVACIDADE, suporta_visao
from branding import (
    PROGRESS_LEGIS_MCID,
    METRICA_LEGIS_MCID,
    SUBHEADER_LEGIS_MCID,
    PORTARIA_RAPIDO_HEADER,
    texto_info_submissao_portaria,
    EXPORT_PORTARIA_DOCX_NAME,
    EXPORT_PORTARIA_MD_NAME,
)
from motor import (
...
def render_gerador_portarias(llm_config: LLMConfig) -> None:
    """Fluxo completo (sub-passos) + assistente RAG só com repositório de portarias."""
    _ia_label = _provider_curto.get(llm_config.provider, llm_config.provider)
    TIPOS_ACEITOS = list(_EXTENSOES_SUPORTADAS.keys())
    TIPOS_LABEL = (
        "PDF, Word (.docx), HTML, Planilhas (.xlsx/.csv), KML/KMZ, "
        "Imagens (.jpg/.png/.tiff)"
    )
    st.title("✏️ Gerador de Portarias")
    st.caption(
        "Redação assistida em fluxo estruturado. O assistente utiliza o documento ativo "
        "e o **Repositório de Portarias** (RAG restrito)."
    )
    ...
    with _col_assistente:
        ...
        render_painel_chat_documento(
            ...
            obter_contexto=_ctx_port_doc,
            key_prefix="gerador_portarias",
            categoria_repositorio="portaria",
        )
